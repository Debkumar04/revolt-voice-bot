...
const VoiceInput = () => {
  ...
  const getLanguageInstruction = () => {
    switch (language) {
      case "hi-IN":
        return "केवल हिंदी में उत्तर दें। अंग्रेजी में अनुवाद न दें।";
      case "mr-IN":
        return "फक्त मराठीत उत्तर द्या. इंग्रजीत भाषांतर करू नका.";
      case "es-ES":
        return "Responde solo en español. No traduzcas al inglés.";
      case "fr-FR":
        return "Répondez uniquement en français. Ne traduisez pas en anglais.";
      default:
        return "Respond only in the user's selected language without English translations.";
    }
  };
  ...
  const speakOutLoud = (text) => {
    ...
    // Filter out English translations for non-English languages
    let filteredText = text;
    if (!language.startsWith("en")) {
      filteredText = text
        .replace(/\(.*?\)/g, "") // Remove parentheses content
        .replace(/\[.*?\]/g, "") // Remove bracket content
        .replace(/English:.*/g, ""); // Remove "English:" lines
    }

    const utterance = new SpeechSynthesisUtterance(filteredText.trim());
...
const sendToBackend = async (promptText) => {
  try {
    const res = await fetch("http://localhost:5000/api/gemini", {
      ...
      body: JSON.stringify({
        prompt: promptText,
        language: language, // Send current UI language
      }),
    });
